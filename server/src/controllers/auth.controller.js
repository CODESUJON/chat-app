...
export const signup=async(req,res)=>{
    try {
        ...
        if(password.length<6){
            return res.status(400).json({message: "Password must be least 6 character"});
        ...
        console.log("error in signup credential",error.message);
...
export const logout=(req,res)=>{ 
   try {
    ...
    res.status(200).json({message : "Logged out successfully"})
     ...
     console.log("error in logout credential", error.message);
